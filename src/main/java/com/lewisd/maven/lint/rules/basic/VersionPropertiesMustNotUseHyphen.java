...
public class VersionPropertiesMustNotUseHyphen extends AbstractRule {
		...
		for (Map.Entry<Object,VersionProperty> entry : versionPropertyByObject.entrySet()) {
			...
			for (String propertyName : versionProperty.getPropertyNames()) {
				if (propertyName.contains("-")) {
					resultCollector.addViolation(mavenProject, "Version property names must not contain a hyphen: '" + propertyName + "'", entry.getKey());
				}
			}
		}
	}
